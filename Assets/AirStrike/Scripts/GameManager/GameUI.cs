...
public class GameUI : MonoBehaviour
{
    ...
    public void OnGUI()
    {
        if (play.SimpleControl != GameManager.SimpleControl)
            if (play.flight)
                play.SimpleControl = GameManager.SimpleControl;
        if (play.Acceleration != GameManager.Acceleration)
            play.Acceleration = GameManager.Acceleration;
        ...
        switch (Mode)
        {
            ...
            case 3:
                ...
                Time.timeScale = 0;
                GUI.skin.label.alignment = TextAnchor.MiddleCenter;
                GUI.Label(new Rect(0, Screen.height / 2 - 50, Screen.width, 30), "Options");

                //GUI.DrawTexture(new Rect(Screen.width / 2 - Logo.width / 2, Screen.height / 2 - 150, Logo.width, Logo.height), Logo);
                if (GUI.Button(new Rect(Screen.width / 2 - 150, Screen.height / 2, 300, 40), "Gyroscope " + GameManager.Acceleration))
                {
                    GameManager.Acceleration = !GameManager.Acceleration;
                }

                if (GUI.Button(new Rect(Screen.width / 2 - 150, Screen.height / 2 +50, 300, 40), "Simple Control " + GameManager.SimpleControl))
                {
                    GameManager.SimpleControl = !GameManager.SimpleControl;
                    //if (play.flight)
                        //play.SimpleControl = !play.SimpleControl;
                }

                if (GUI.Button(new Rect(Screen.width / 2 - 150, Screen.height / 2 + 100, 300, 40), "Change View"))
                {
                    if (play.View)
                        play.View.SwitchCameras();
                }

                //if (GUI.Button(new Rect(Screen.width / 2 - 150, Screen.height / 2 + 100, 300, 40), "Change Weapons"))
                //{
                //    if (play.flight)
                //        play.flight.WeaponControl.SwitchWeapon();
                //}
